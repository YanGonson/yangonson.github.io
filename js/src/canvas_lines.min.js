...
THREE.CanvasRenderer = function(e) {
    ...
    function ae(e, t, i, r, n, o, a, s) {
        ...
        !0 === s.wireframe ? se(G, s.wireframeLinewidth, s.wireframeLinecap, s.wireframeLinejoin) : le(G);
            ...
            if (null !== s.map) s.map.mapping === THREE.UVMapping && (T = a.uvs, pe(u, m, v, x, R, y, T[r].x, T[r].y, T[n].x, T[n].y, T[o].x, T[o].y, s.map));
            else null !== s.envMap ? s.envMap.mapping === THREE.SphericalReflectionMapping && (ie.copy(a.vertexNormalsModel[r]).applyMatrix3(re), w = .5 * ie.x + .5, H = .5 * ie.y + .5, ie.copy(a.vertexNormalsModel[n]).applyMatrix3(re), g = .5 * ie.x + .5, S = .5 * ie.y + .5, ie.copy(a.vertexNormalsModel[o]).applyMatrix3(re), M = .5 * ie.x + .5, b = .5 * ie.y + .5, pe(u, m, v, x, R, y, w, H, g, S, M, b, s.envMap)) : (G.copy(s.color), s.vertexColors === THREE.FaceColors && G.multiply(a.color), !0 === s.wireframe ? se(G, s.wireframeLinewidth, s.wireframeLinecap, s.wireframeLinejoin) : le(G))
        } else s instanceof THREE.MeshNormalMaterial ? (ie.copy(a.normalModel).applyMatrix3(re), G.setRGB(ie.x, ie.y, ie.z).multiplyScalar(.5).addScalar(.5)) : G.setRGB(1, 1, 1),
        !0 === s.wireframe ? se(G, s.wireframeLinewidth, s.wireframeLinecap, s.wireframeLinejoin) : le(G)
    }
    function se(e, t, i, r) {
        fe(t),
    ...
    function ce(e) {
        ...
        var i = e.wrapS === THREE.RepeatWrapping || e.wrapS === THREE.MirroredRepeatWrapping,
        r = e.wrapT === THREE.RepeatWrapping || e.wrapT === THREE.MirroredRepeatWrapping,
        ...
        var l = "no-repeat"; ! 0 === i && !0 === r ? l = "repeat": !0 === i ? l = "repeat-x": !0 === r && (l = "repeat-y");
    ...
    function pe(e, t, i, r, n, o, a, s, l, c, p, E, d) {
        var h = J[d.id];
        if (void 0 !== h && h.version === d.version || (h = ce(d), J[d.id] = h), void 0 === h.canvas) return xe("rgba( 0, 0, 0, 1)"),
        void P.fill();
        xe(h.canvas);
        var f, u, m, v, x, R, y, T, w = d.offset.x / d.repeat.x,
        ...
        n -= e,
        o -= t,
        0 !== (y = (l -= a = (a + w) * g) * (E -= s = (s + H) * S) - (p -= a) * (c -= s)) && (x = e - (f = (E * i - c * n) * (T = 1 / y)) * a - (m = (l * n - p * i) * T) * s, R = t - (u = (E * r - c * o) * T) * a - (v = (l * o - p * r) * T) * s, P.save(), P.transform(f, u, m, v, x, R), P.fill(), P.restore())
    }
    function Ee(e, t, i) {
        var r, n = t.x - e.x,
        o = t.y - e.y,
        a = n * n + o * o;
        0 !== a && (n *= r = i / Math.sqrt(a), o *= r, t.x += n, t.y += o, e.x -= n, e.y -= o)
    }
    function de(e) {
    ...
    this.setSize = function(e, t, i) {
        ...
        V = Math.floor(h / 2),
        j = Math.floor(z / 2),
        !1 !== i && (r.style.width = e + "px", r.style.height = t + "px"),
        Y.min.set( - V, -j),
        Y.max.set(V, j),
        $.min.set( - V, -j),
        $.max.set(V, j),
    ...
    this.clear = function() { ! 1 === $.isEmpty() && ($.intersect(Y), $.expandByScalar(2), $.min.x = $.min.x + V, $.min.y = -$.min.y + j, $.max.x = $.max.x + V, $.max.y = -$.max.y + j, o < 1 && P.clearRect(0 | $.min.x, 0 | $.max.y, $.max.x - $.min.x | 0, $.min.y - $.max.y | 0), 0 < o && (he(THREE.NormalBlending), de(1), xe("rgba(" + Math.floor(255 * i.r) + "," + Math.floor(255 * i.g) + "," + Math.floor(255 * i.b) + "," + o + ")"), P.fillRect(0 | $.min.x, 0 | $.max.y, $.max.x - $.min.x | 0, $.min.y - $.max.y | 0)), $.makeEmpty())
    },
    ...
    this.render = function(e, t) {
        if (t instanceof THREE.Camera != !1) {
            var i = e.background;
            i && i.isColor ? (xe("rgb(" + Math.floor(255 * i.r) + "," + Math.floor(255 * i.g) + "," + Math.floor(255 * i.b) + ")"), P.fillRect(0, 0, h, z)) : !0 === this.autoClear && this.clear(),
            C.info.render.vertices = 0,
            C.info.render.faces = 0,
            P.setTransform(B / h, 0, 0, -W / z, L, z - O),
            P.translate(V, j),
            ...
            function() {
                ...
                t = f.length; e < t; e++) {
                    var i = f[e],
                    r = i.color;
                    i instanceof THREE.AmbientLight ? Q.add(r) : i instanceof THREE.DirectionalLight ? Z.add(r) : i instanceof THREE.PointLight && _.add(r)
                }
            } ();
            for (var r = 0,
            n = l.length; r < n; r++) {
                ...
                if (void 0 !== a && 0 !== a.opacity) {
                    if (K.makeEmpty(), o instanceof THREE.RenderableSprite)(c = o).x *= V,
                    c.y *= j,
                    ne(c, o, a);
                    ...
                    !0 === Y.intersectsBox(K) && oe(c, p, o, a);
                        ...
                        K.setFromPoints([c.positionScreen, p.positionScreen, E.positionScreen]),
                        !0 === Y.intersectsBox(K) && ae(c, p, E, 0, 1, 2, o, a)
                    }
                    $.union(K)
                }
            }
...
THREE.Projector = function() {
    var K, Q, n, Z, _, ee, te, ie, re, ne, oe, ae = [],
    se = 0,
    le = [],
    t = 0,
    i = [],
    ...
    function() {
        ...
        function r(e) {
            ...
            i = e.positionWorld,
            r = e.positionScreen;
            i.copy(t).applyMatrix4(oe),
            r.copy(i).applyMatrix4(he);
            var n = 1 / r.w;
            r.x *= n,
            r.y *= n,
            r.z *= n,
            e.visible = -1 <= r.x && r.x <= 1 && -1 <= r.y && r.y <= 1 && -1 <= r.z && r.z <= 1
        }
        function h(e, t, i) {
            return ! 0 === e.visible || !0 === t.visible || !0 === i.visible || (v[0] = e.positionScreen, v[1] = t.positionScreen, v[2] = i.positionScreen, u.intersectsBox(m.setFromPoints(v)))
        }
        function f(e, t, i) {
        ...
        return {
            ...
            pushVertex: function(e, t, i) { (n = ye()).position.set(e, t, i),
                r(n)
            },
            ...
            pushLine: function(e, t) {
                var i = le[e],
                r = le[t]; (te = we()).id = p.id,
                te.v1.copy(i),
                te.v2.copy(r),
                te.z = (i.positionScreen.z + r.positionScreen.z) / 2,
            ...
            pushTriangle: function(e, t, i) {
                ...
                n = le[t],
                o = le[i];
                if (!1 !== h(r, n, o) && (E.side === THREE.DoubleSide || !0 === f(r, n, o))) { (_ = Te()).id = p.id,
                    _.v1.copy(r),
                    _.v2.copy(n),
                    _.v3.copy(o),
                    _.z = (r.positionScreen.z + n.positionScreen.z + o.positionScreen.z) / 3,
    ...
    function Se(e, t) {
        ...
        a = -e.z + e.w,
        s = -t.z + t.w;
        return 0 <= n && 0 <= o && 0 <= a && 0 <= s || !(n < 0 && o < 0 || a < 0 && s < 0) && (n < 0 ? i = Math.max(i, n / (n - o)) : o < 0 && (r = Math.min(r, n / (n - o))), a < 0 ? i = Math.max(i, a / (a - s)) : s < 0 && (r = Math.min(r, a / (a - s))), !(r < i) && (e.lerp(t, i), t.lerp(e, 1 - r), !0))
    }
    this.projectScene = function(e, t, i, r) {
        function n(e) { (K = function() {
                if (Q === se) {
        ...
        he.multiplyMatrices(t.projectionMatrix, de),
        me.setFromMatrix(he),
        Q = 0,
        ce.objects.length = 0,
        ce.lights.length = 0,
        ...
        a = ce.objects.length; o < a; o++) {
            ...
            if (Re.setObject(s), oe = s.matrixWorld, Z = 0, s instanceof THREE.Mesh) {
                if (l instanceof THREE.BufferGeometry) {
                    ...
                    for (var E = 0,
                    d = (J = c.position.array).length; E < d; E += 3) Re.pushVertex(J[E], J[E + 1], J[E + 2]);
                    if (void 0 !== c.normal) {
                        var h = c.normal.array;
                        for (E = 0, d = h.length; E < d; E += 3) Re.pushNormal(h[E], h[E + 1], h[E + 2])
                    }
                    if (void 0 !== c.uv) {
                        var f = c.uv.array;
                        for (E = 0, d = f.length; E < d; E += 2) Re.pushUv(f[E], f[E + 1])
                    }
                    if (null !== l.index) {
                        var u = l.index.array;
                    ...
                    } else for (E = 0, d = J.length / 3; E < d; E += 3) Re.pushTriangle(E, E + 1, E + 2)
                    ...
                    B = R.length; O < B; O++) {
                        ...
                        if (void 0 !== (T = !0 === w ? H.materials[W.materialIndex] : s.material)) {
                            ...
                            if (!1 !== Re.checkTriangleVisibility(k, N, F)) {
                                ...
                                var q = y[O];
                                if (void 0 !== q) for (var X = 0; X < 3; X++) _.uvs[X].copy(q[X]);
                                _.color = W.color,
                                _.material = T,
                ...
                if (l instanceof THREE.BufferGeometry) {
                    if (void 0 !== (c = l.attributes).position) {
                        var J;
                        for (E = 0, d = (J = c.position.array).length; E < d; E += 3) Re.pushVertex(J[E], J[E + 1], J[E + 2]);
                        if (null !== l.index) for (E = 0, d = (u = l.index.array).length; E < d; E += 2) Re.pushLine(u[E], u[E + 1]);
                        else {
                            var Y = s instanceof THREE.LineSegments ? 2 : 1;
                            for (E = 0, d = J.length / 3 - 1; E < d; E += Y) Re.pushLine(E, E + 1)
                        }
                    }
                } else if (l instanceof THREE.Geometry) {
                    if (fe.multiplyMatrices(he, oe), 0 === (x = s.geometry.vertices).length) continue; (k = ye()).positionScreen.copy(x[0]).applyMatrix4(fe);
                    for (Y = s instanceof THREE.LineSegments ? 2 : 1, g = 1, S = x.length; g < S; g++)(k = ye()).positionScreen.copy(x[g]).applyMatrix4(fe),
                ...
                var $ = 1 / Ee.w;
                Ee.z *= $,
                -1 <= Ee.z && Ee.z <= 1 && ((re = He()).id = s.id, re.x = Ee.x * $, re.y = Ee.y * $, re.z = Ee.z, re.renderOrder = s.renderOrder, re.object = s, re.rotation = s.rotation, re.scale.x = s.scale.x * Math.abs(re.x - (Ee.x + t.projectionMatrix.elements[0]) / (Ee.w + t.projectionMatrix.elements[12])), re.scale.y = s.scale.y * Math.abs(re.y - (Ee.y + t.projectionMatrix.elements[5]) / (Ee.w + t.projectionMatrix.elements[13])), re.material = s.material, ce.elements.push(re))
            }
        }
        return ! 0 === r && ce.elements.sort(ge),
        ce
    }
};